...
class ModelArguments:
    ...
    # Train PinPoint or not
    train_pinpoint: bool = field(default=False)
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    if hasattr(trainer.args, "train_pinpoint"):
        check_only_save_mm_adapter_tunnable = False
...
def preprocess_v1(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    ...
    for i, source in enumerate(sources):
        ...
        human_prompt_value = source[0]['value'].replace('<image>\n', '').strip()
        # print(human_prompt_value)
        tokenized_prompt = tokenizer(
            human_prompt_value,
            return_tensors=None,
            add_special_tokens=False
        ).input_ids
        ques_token.append(tokenized_prompt)
    ...
    if has_image:
        input_ids = torch.stack([tokenizer_image_token(prompt, None, tokenizer, return_tensors="pt") for prompt in conversations], dim=0)
    else:
        input_ids = tokenizer(
            conversations,
            return_tensors="pt",
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            if has_image:
                round_len = len(tokenizer_image_token(rou, None, tokenizer))
                instruction_len = len(tokenizer_image_token(parts[0], None, tokenizer)) - 2
            else:
                round_len = len(tokenizer(rou).input_ids)
                instruction_len = len(tokenizer(parts[0]).input_ids) - 2
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            image_folder = self.data_args.image_folder
            image_org = Image.open(os.path.join(image_folder, image_file)).convert("RGB")
            image_size = image_org.size
        ...
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0], ques_token=data_dict["ques_token"][0])

        # image exist in the data
        if "image" in self.list_data_dict[i]:
            data_dict["image"] = image
            data_dict["image_size"] = torch.tensor(image_size) # (Width x Hight)
            data_dict["image_org"] = image_org
        ...
        if "answer_bbox" in self.list_data_dict[i]:
            data_dict["answer_bbox"] = self.list_data_dict[i]["answer_bbox"]
        if "encompass_bbox" in self.list_data_dict[i]:
            data_dict["encompass_bbox"] = self.list_data_dict[i]["encompass_bbox"]
        
        data_dict["train_pinpoint"] = True
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "ques_token" in instances[0]:
            batch["ques_tokens"] = [instance["ques_token"] for instance in instances]

        if "image_org" in instances[0]:
            batch["image_orgs"] = [instance["image_org"] for instance in instances]

        if "image_size" in instances[0]:
            batch["image_org_sizes"] = torch.stack([instance["image_size"] for instance in instances])

        if "answer_bbox" in instances[0]:
            batch["answer_bboxes"] = [instance.get("answer_bbox") for instance in instances]

        if "encompass_bbox" in instances[0]:
            batch["encompass_bboxes"] = [instance.get("encompass_bbox") for instance in instances]

        if "train_pinpoint" in instances[0]:
            batch["train_pinpoint"] = [instance.get("train_pinpoint") for instance in instances]
    ...
    if model_args.vision_tower is not None:
        ...
        if data_args.image_grid_pinpoints is not None:
            if isinstance(data_args.image_grid_pinpoints, str) and "x" in data_args.image_grid_pinpoints:
                ...
                # Multiply all elements by patch_sizer
        ...
        if model_args.train_pinpoint:
            model.config.train_pinpoint = training_args.train_pinpoint = model_args.train_pinpoint
            for name, param in model.named_parameters():
                if 'pinpoint' in name:
                    param.requires_grad = True

        elif model_args.mm_tunable_parts is None and model_args.train_pinpoint is None:  # traditional way of deciding which part to train
