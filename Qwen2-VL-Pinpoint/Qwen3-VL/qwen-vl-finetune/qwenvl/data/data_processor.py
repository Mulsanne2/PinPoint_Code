...
def preprocess_qwen_visual(
    ...
    original_ques = messages[0]['content'][1]['text']

    image_path = os.path.join(base_path, source['image'])
    image_org = Image.open(image_path)
    image_org_size = image_org.size
    answer_bbox = source['answer_bbox']
    encompass_bbox = source['encompass_bbox']
    
    full_result = processor.apply_chat_template(
        messages, tokenize=True, return_dict=True, return_tensors="pt"
    )
    ques_ids = processor.tokenizer([original_ques])['input_ids'][0]

    input_ids = full_result["input_ids"]
    if isinstance(input_ids, list):
        input_ids = torch.tensor(input_ids).unsqueeze(0)
    if isinstance(ques_ids, list):
        ques_ids = torch.tensor(ques_ids).unsqueeze(0)
    ...
    full_result["ques"] = ques_ids
    full_result["image_org_size"] = image_org_size
    full_result["image_org"] = image_org
    full_result["answer_bbox"] = answer_bbox
    full_result["encompass_bbox"] = encompass_bbox
    return full_result
...
class FlattenedDataCollatorForSupervisedDataset(DataCollatorForSupervisedDataset):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        answer_bboxes = [
            instance["answer_bbox"]
            for instance in instances
            if "answer_bbox" in instance
        ]
        batch["answer_bbox"] = answer_bboxes

        encompass_bboxes = [
            instance["encompass_bbox"]
            for instance in instances
            if "encompass_bbox" in instance
        ]
        batch["encompass_bbox"] = encompass_bboxes

        image_orgs = [
            instance["image_org"]
            for instance in instances
            if "image_org" in instance
        ]
        batch["image_org"] = image_orgs

        image_org_sizes = [
            instance["image_org_size"]
            for instance in instances
            if "image_org_size" in instance
        ]
        batch["image_org_size"] = image_org_sizes
        ...
        batch["ques"] = questions
        batch["train_pinpoint"] = True
        return batch
